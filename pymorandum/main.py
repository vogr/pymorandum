...
def create_thumbnails(pictures_to_process, dest, icc_profile=None, delete_icc=False):
    ...
    for p in pictures_to_process:
        shutil.copy(p, dest / p.name / Path('original'))
...
def main():
    ...
    outfile_hashes = {}
    for d in (d for d in outdir.iterdir() if d.is_dir()):
        if not (indir / (d.relative_to(outdir))).exists():
            shutil.rmtree(d)
        for subdir in d.iterdir():
            key = subdir.relative_to(outdir)
            if not (indir / key).exists():
    ...
    for d in photo_dirs:
        pictures_to_process = []
        for picture in (p for p in d.iterdir() if
                        p.is_file() and p.suffix in photo_exts):
            key = picture.relative_to(indir)
            ...
            if h is not None:
                if h != gen_hash(picture):
                    ...
                    pictures_to_process.append(picture)
        create_thumbnails(pictures_to_process, outdir / d,
